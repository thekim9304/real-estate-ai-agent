...
def process_query(query: str):
    ...
    # pp.pprint(query_structure)

    if query_structure.get('intent') == '해당 없음':
        return "죄송합니다, 해당 질문은 제가 답변하기 어려운 유형입니다. 부동산(아파트 혹은 오피스텔) 거래 정보에 대해 질문해주세요."
    try:
        # 1-2) Fill bjd name
        ...
        if not sd_sgg_dong_lst:
            return "죄송합니다, 정확한 지역(or건물)명을 입력해주세요. 현재 저희 서비스는 수도권(서울, 경기) 지역의 정보만 제공하고 있습니다."
        ...
        if filtered_pdf.empty:
            return "요청하신 조건에 맞는 거래 내역을 찾지 못했습니다."

        # 답변 생성
        res_data_json = prepare_data_for_llm_by_pyeong(query_structure['intent'], filtered_pdf)
        final_resp = gen_final_response(client, query_structure, res_data_json)
        return final_resp
